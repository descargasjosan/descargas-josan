...
const App: React.FC = () => {
  ...
  const handleAddFuel = () => {
      ...
      if (!newFuelRecord.liters || !newFuelRecord.cost) {
          showNotification("Litros y Coste son obligatorios", "error");
          return;
      }
      const record: FuelRecord = {
          ...
          liters: parseFloat(newFuelRecord.liters),
           ...
           <div className="flex-1 bg-slate-50 overflow-y-auto p-8 custom-scrollbar">
             <div className="flex items-center justify-between mb-8">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   <h2 className="text-2xl font-black text-slate-900 italic uppercase">Cartera de Clientes</h2>
                    ...
                    <section className="bg-amber-50/50 p-6 rounded-3xl border border-amber-100">
                        ...
                        <div className="bg-white p-4 rounded-2xl border border-amber-100 mb-4">
                            <div className="grid grid-cols-3 gap-3 mb-3">
                                ...
                                <input type="number" step="0.01" placeholder="Litros" className="bg-slate-50 rounded-lg px-2 py-1.5 text-xs font-bold" value={newFuelRecord.liters} onChange={e => setNewFuelRecord({...newFuelRecord, liters: e.target.value})} />
                        ...
                        <div className="max-h-[150px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                             {planning.fuelRecords?.filter(fr => fr.workerId === editingWorker.id).map(fr => (
                                 <div key={fr.id} className="flex justify-between items-center p-2 bg-white rounded-xl border border-slate-100 text-xs">
                                     ...
                                     <span className="font-black text-slate-800">{fr.liters}L - {fr.cost}€</span>
